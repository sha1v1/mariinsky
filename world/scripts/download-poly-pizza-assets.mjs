...
const assets = [
  ...
  { id: "bridge", category: "miscellaneous", modelId: "j4KsIuJYnq" },
  { id: "fence", category: "miscellaneous", modelId: "UXmKfG81fG" },
  { id: "mailbox", category: "miscellaneous", modelId: "FCuds5nBko" },
  { id: "well", category: "miscellaneous", modelId: "QlqncKYxXb" },
  { id: "fountain", category: "miscellaneous", modelId: "WHc7dwttlk" },
];

const requested = new Set(process.argv.slice(2));
const queue = requested.size > 0 ? assets.filter((asset) => requested.has(asset.id)) : assets;
if (requested.size > 0 && queue.length !== requested.size) {
  const known = new Set(assets.map((asset) => asset.id));
  throw new Error(`Unknown asset id(s): ${[...requested].filter((id) => !known.has(id)).join(", ")}`);
}
