...
/**
 * Phrase-level shredding, in descending order of how strong a break is: line
 * breaks, then sentence ends, then internal punctuation. Whatever is still too
 * long after that is wrapped greedily at word boundaries, so no fragment ever
 * exceeds the caps -- a fragment you can read in one glance is the whole point.
 * Words longer than the character cap are left whole rather than cut in half.
 */
export function splitText(text, limits) {
  const { maxWords, maxChars, minWords } = { ...TEXT_LIMITS, ...(limits || {}) };
  const out = [];
  for (const line of String(text || '').split(/\r?\n+/)) {
    ...
    for (const sentence of clean.split(/(?<=[.!?…])\s+/)) {
      for (const clause of sentence.split(/(?<=[,;:—–])\s+|\s+(?=[—–])/)) {
        wrapInto(out, clause.trim(), maxWords, maxChars);
      }
    }
  }
  return mergeScraps(out, maxWords, maxChars, minWords);
}

function wrapInto(out, clause, maxWords, maxChars) {
  if (!clause) return;
  let buf = [];
  const flush = () => { if (buf.length) out.push(buf.join(' ')); buf = []; };
  for (const w of clause.split(' ').filter(Boolean)) {
    if (buf.length) {
      const next = buf.concat(w);
      if (next.length > maxWords || next.join(' ').length > maxChars) flush();
    }
    buf.push(w);
  }
  flush();
...
/**
 * A stray one-word scrap is folded back into the fragment before it -- but only
 * when the result still fits. Otherwise it is left to stand on its own, which
 * in a memory is no bad thing.
 */
function mergeScraps(frags, maxWords, maxChars, minWords) {
  const merged = [];
  for (const frag of frags) {
    const prev = merged.at(-1);
    if (prev && frag.split(' ').length < minWords) {
      const joined = `${prev} ${frag}`;
      if (joined.split(' ').length <= maxWords && joined.length <= maxChars) {
